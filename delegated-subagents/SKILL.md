...
# Public-safe export is the default: coarse monthly/provider bands only
scripts/dashboard-export.sh --output /abs/path/to/delegated-usage.json

# Detailed operational export is explicit and local/private only
...
coverage. The public dashboard exporter never invokes a model and emits only
thresholded month/provider bands. It suppresses samples below 10 attempts and
excludes prompts, transcripts, repository paths, attempts, exact models,
task/result labels, billing, costs, exact token counts, exact timestamps,
session identifiers, command lines, and raw errors. Detailed schema version 1
requires `--privacy private` and must never be committed to a public repo.
...
- `scripts/dashboard-export.sh`: public-safe aggregate export by default; explicit private operational export.
