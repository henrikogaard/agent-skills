...
PUBLIC_DASHBOARD_MINIMUM_ATTEMPTS = 10
PUBLIC_DASHBOARD_PROVIDERS = {"cursor", "devin", "opencode"}
...
def _coverage_band(measured: int, attempts: int) -> str:
    if attempts <= 0 or measured <= 0:
        return "0%"
    percent = measured * 100 / attempts
    if percent >= 100:
        return "100%"
    if percent >= 75:
        return "75-99%"
...
def _tokens_band(value: int | float) -> str:
    if value <= 0:
        return "0"
    if value < 1_000:
        return "1-999"
...
def _public_aggregate_row(label_field: str, label: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    attempts = len(rows)
    measured = sum(int(bool(row.get("usage_available"))) for row in rows)
    tokens = sum((_number(row.get("total_tokens")) or 0) for row in rows)
    return {
        label_field: label,
        "attempts_band": _count_band(attempts),
        "capture_coverage_band": _coverage_band(measured, attempts),
        "delegated_tokens_band": _tokens_band(tokens),
    }


def build_public_dashboard_snapshot(args: argparse.Namespace) -> dict[str, Any]:
    attempts = _safe_attempt_rows(args.state_root)
    eligible = [row for row in attempts if row.get("provider") in PUBLIC_DASHBOARD_PROVIDERS]
    total_attempts = len(eligible)
    sufficient = total_attempts >= PUBLIC_DASHBOARD_MINIMUM_ATTEMPTS
    generated_month = utc_now()[:7]

    providers: list[dict[str, Any]] = []
    months: list[dict[str, Any]] = []
    if sufficient:
        for provider in sorted(PUBLIC_DASHBOARD_PROVIDERS):
            rows = [row for row in eligible if row.get("provider") == provider]
            if len(rows) >= PUBLIC_DASHBOARD_MINIMUM_ATTEMPTS:
                providers.append(_public_aggregate_row("provider", provider, rows))
        ...
        month_labels = sorted(
            {
                ...
                and re.fullmatch(r"\d{4}-\d{2}-.+", str(row["timestamp"]))
            }
        )
        for month in month_labels:
            rows = [row for row in eligible if str(row.get("timestamp", ""))[:7] == month]
            if len(rows) >= PUBLIC_DASHBOARD_MINIMUM_ATTEMPTS:
                months.append(_public_aggregate_row("month", month, rows))

    measured = sum(int(bool(row.get("usage_available"))) for row in eligible)
    tokens = sum((_number(row.get("total_tokens")) or 0) for row in eligible)
    return {
        "schema_version": 2,
        "privacy": "public",
        "data_kind": "aggregate",
        ...
        "summary": {
            ...
            "attempts_band": _count_band(total_attempts) if sufficient else "suppressed",
            "capture_coverage_band": _coverage_band(measured, total_attempts) if sufficient else "suppressed",
            "delegated_tokens_band": _tokens_band(tokens) if sufficient else "suppressed",
        },
        "providers": providers,
        "months": months,
    }
...
def validate_public_dashboard_snapshot(snapshot: dict[str, Any]) -> None:
    expected_fields = {
        "schema_version",
        "privacy",
        "data_kind",
    ...
    if set(snapshot) != expected_fields:
        raise ValueError("unexpected public dashboard fields")
    if snapshot.get("schema_version") != 2 or snapshot.get("privacy") != "public":
        raise ValueError("unsupported public dashboard schema")
    if snapshot.get("data_kind") not in {"aggregate", "synthetic"}:
        raise ValueError("unsupported public dashboard data kind")
    if not re.fullmatch(r"\d{4}-\d{2}", str(snapshot.get("generated_month", ""))):
        raise ValueError("public dashboard generated_month must be YYYY-MM")
    if not isinstance(snapshot.get("summary"), dict):
        raise ValueError("public dashboard summary must be an object")
    if not isinstance(snapshot.get("providers"), list) or not isinstance(snapshot.get("months"), list):
        raise ValueError("public dashboard aggregates must be arrays")
    if any(row.get("provider") not in PUBLIC_DASHBOARD_PROVIDERS for row in snapshot["providers"] if isinstance(row, dict)):
        raise ValueError("public dashboard contains an unsupported provider")
...
def dashboard_export_command(args: argparse.Namespace) -> int:
    try:
        if args.privacy == "private":
            ...
            snapshot = build_public_dashboard_snapshot(args)
            validate_public_dashboard_snapshot(snapshot)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    dashboard = subparsers.add_parser("dashboard-export", help="write a privacy-scoped static dashboard snapshot")
    ...
    dashboard.add_argument(
        ...
        default="public",
        help="public writes coarse thresholded aggregates; private retains detailed local-only telemetry",
    )
